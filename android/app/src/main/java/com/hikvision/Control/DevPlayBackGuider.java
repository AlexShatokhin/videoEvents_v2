...
import com.hikvision.jna.PlayCtrlJNA;
...
public class DevPlayBackGuider {
    ...
    public boolean PlaybackSetPrivateData(int lRealHandle, int iIntelType,int iSubType,int bTrue){
        if (lRealHandle < 0) {
            Log.e("SimpleDemo", "PLaybackSetPrivateData failed with error param");
            return false;
        }
        return HCNetSDKJNAInstance.getInstance().NET_DVR_RenderPrivateDataEx(lRealHandle, iIntelType, iSubType, bTrue);
        //return  HCNetSDK.getInstance().NET_DVR_RenderPrivateDataEx(lRealHandle, iIntelType, iSubType, bTrue);
    }
    ...
    public int PlayBackByName_jni(int iLogID, String strFileName, Surface surface){
        ...
        disableOSD(iPlaybackID);
        return iPlaybackID;
    }

    private static final int[] PLAYM4_RENDER_TYPES = {
            0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80
    };

    private void disableOSD(int iPlaybackHandle) {
        try {
            int nPort = HCNetSDKJNAInstance.getInstance().NET_DVR_GetPlayBackPlayerIndex(iPlaybackHandle);
            if (nPort >= 0) {
                for (int type : PLAYM4_RENDER_TYPES) {
                    PlayCtrlJNA.INSTANCE.PlayM4_RenderPrivateData(nPort, type, 0);
                    PlayCtrlJNA.INSTANCE.PlayM4_RenderPrivateDataEx(nPort, type, 0, 0);
                }
            }
        } catch (Throwable e) {
            Log.w("[NetSDKSimpleDemo]", "disableOSD failed: " + e.getMessage());
        }
    }
    ...
    public boolean PausePlayBack_jni(int iPlaybackID){
        ...
        return HCNetSDK.getInstance().NET_DVR_PlayBackControl_V40(iPlaybackID, HCNetSDK.NET_DVR_PLAYPAUSE, null, 0, null);
    }

    public boolean ContinuePlayBack_jni(int iPlaybackID){
        ...
        return HCNetSDK.getInstance().NET_DVR_PlayBackControl_V40(iPlaybackID, HCNetSDK.NET_DVR_PLAYRESTART, null, 0, null);
    }
...
//        }
    ...
    public int PlayBackPauseByTime_v40_jni(int iLogID, int iPlaybackID){
        if(iLogID < 0 || iPlaybackID < 0){
            Log.e("SimpleDemo", "PlayBackByTime_v40_jni failed with error param");
            return -1;
        }

        if (!HCNetSDK.getInstance().NET_DVR_PlayBackControl_V40(iPlaybackID, PlaybackControlCommand.NET_DVR_PLAYPAUSE, null, 0, null))
        {
            Log.e("SimpleDemo", "NET_DVR_PlayBackControl_V40 is failed!Err:" + HCNetSDK.getInstance().NET_DVR_GetLastError());
            HCNetSDK.getInstance().NET_DVR_StopPlayBack(iPlaybackID);
            return -1;
...
//        if (!HCNetSDK.getInstance().NET_DVR_PlayBackControl_V40(iPlaybackID, PlaybackControlCommand.NET_DVR_PLAYSTOPAUDIO, null, 0, null))
//        {
//            Log.e("SimpleDemo", "NET_DVR_PlayBackControl_V40 is failed!Err:" + HCNetSDK.getInstance().NET_DVR_GetLastError());
//            HCNetSDK.getInstance().NET_DVR_StopPlayBack(iPlaybackID);
//            return -1;
//        }
    ...
    public int PlayBackContinueByTime_v40_jni(int iLogID, int iPlaybackID){
        if(iLogID < 0 || iPlaybackID < 0){
            Log.e("SimpleDemo", "PlayBackByTime_v40_jni failed with error param");
            return -1;
        }
        ...
        if (!HCNetSDK.getInstance().NET_DVR_PlayBackControl_V40(iPlaybackID, PlaybackControlCommand.NET_DVR_PLAYRESTART, null, 0, null))
        {
            Log.e("SimpleDemo", "NET_DVR_PlayBackControl_V40 is failed!Err:" + HCNetSDK.getInstance().NET_DVR_GetLastError());
            HCNetSDK.getInstance().NET_DVR_StopPlayBack(iPlaybackID);
            return -1;
